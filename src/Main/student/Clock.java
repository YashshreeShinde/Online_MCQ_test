...
class Run extends JPanel implements Runnable
{
	public void run() {
		...
		if(Clock.HH==00 && Clock.MM==00 && Clock.SS==00)
		{
	...
	public void makeGUI(String txt) {
		setLayout(null);
		setBackground(new Color(42, 145, 176));
		JLabel lblXxxxxx = new JLabel(txt);
		lblXxxxxx.setBackground(new Color(148, 0, 211));
		lblXxxxxx.setForeground(Color.DARK_GRAY);
		lblXxxxxx.setHorizontalAlignment(SwingConstants.LEFT);
		lblXxxxxx.setFont(new Font("Times Roman", Font.PLAIN, 30));
		lblXxxxxx.setBounds(0, 0, 150, 90);
		add(lblXxxxxx);
		repaint();
		revalidate();
		try {
